...
You probably need to know how to use the Linux terminal, and what
...
Note:
  ...
  Update 2025: There's Nixpkgs too!
- You don't need to install anything on your computer, except for Docker (and some
  standard tools like Git and Make).
...
- However, to run end-to-end tests, you currently need to run Webdriver.io directly on your
  ...
  (So you'll need Nodejs, or Nix-pkgs. More about this in
  the [end-to-end-tests readme](e2e-tests-readme.md).)
  -->

(In addition to `sudo docker compose <sth>...` there's also `sudo docker <sth-else>...` but that's
more low-level.)
...
After this, everything will hopefully be okay again. If not, you can also delete
all Docker volumes, see *Where're the database files* below, and rebuild the images,
see below.
...
There're more volumes in `./volumes`, and inside `docker-compose.yml` the
...
Here's how to view logs:
    ...
    docker compose logs -f          # tails all logs
    docker compose logs -f app      # tails only the 'app' container's logs
