...
-- "squashed" into just one single migration, starting at version nr 1.
...
--
-- Then 2) processed this dump in Vim like so: (to remove comments and empty lines)
--   '<,'>s/^--.*$//g
--   %s/\n\n\n\n\n\n\n/\r\r\r/g
--
-- And then 3) appended a --data-only  _data_only_dump, at the bottom. (The Flyway
-- migrations included not only DDL, but 2 DML statements too.)
--
-- And 4) rewrote a COPY to `insert into` — the COPY data line resulted in an error.
--
-- And 5) commented out some SET etc, not needed.
...
insert into public.system_settings_t (
...
-- But this, generated by pg_dump, results in an error:  'syntax error at or near "\"'
/*
