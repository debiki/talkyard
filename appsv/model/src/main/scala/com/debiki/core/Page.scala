...
object PageType {
  ...
  /** Like OpenChat, but one doesn't need to join the chat, to post a message.
    ...
    * Can change page role only to OpenChat.
    */
  // Maybe rename to NoJoinChat or AnyoneChat — anyone may post, without having joined first
  case object JoinlessChat extends PageType(22, staffOnly = false) {
    override def isChat = true
    override def isOpenChat = false
  }
  ...
  /** Any forum member who can see this chat (i.e. can see the category
    ...
    * Can change page role only to JoinlessChat.
    */
  // Maybe rename to JoinFirstChat or just Chat?  One first needs to join the chat channel.
