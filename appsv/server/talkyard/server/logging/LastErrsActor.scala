...
/** Error = 20, warning = 30?   See blog-comments.ts  [ty_log_levels]
  */
case class LogMsg(msg: St, level: i32) //, anyThrowable: Opt[Throwable])
...
object LastErrsActor {
  ...
  def startNewActor(globals: Globals): ActorRef = {
    ...
    // For now, email any errors after startup, then every 20 minutes.
    globals.actorSystem.scheduler.scheduleWithFixedDelay(
        initialDelay = 5*60 seconds, 20*60 seconds, actorRef, EmailErrorsToOnCalls)
...
class LastErrsActor(val globals: Globals) extends Actor {
  ...
  /** Don't use TyLogger — it'd make this actor sends messages to itself about bugs
    ...
    */
  private val logger: org.slf4j.Logger = // play.api.Logger =
