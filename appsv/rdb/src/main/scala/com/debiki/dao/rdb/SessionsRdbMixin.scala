...
trait SessionsRdbMixin extends SiteTransaction {
  ...
  def loadSession(part1Maybe2Or3: Opt[St] = None, part4HttpOnly: Opt[St] = None)
    ...
    val idPart = part4HttpOnly.map(hashSha512_256ToBytesLen32) getOrElse {
      // We want part 1 only (not 2 or 3).
    ...
    val query = s"""
          ...
          where site_id_c = ? and $colName = ? """
    runQueryFindOneOrNone(query, List(siteId.asAnyRef, idPart), parseSession)
  }
  ...
  def loadOneOrTwoSessions(part1Maybe2Or3: Opt[St], part4HttpOnly: Opt[St])
    ...
    val partFourEq = part4HttpOnly map { part4 =>
      values.append(hashSha512_256ToBytesLen32(part4))
      "hash_4_http_only_c = ?"
    } getOrElse ""

    val query = s"""
          select * from sessions_t
          where site_id_c = ? and ($partOneEq $or $partFourEq) """
