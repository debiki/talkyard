...
class RdbSystemTransaction(
  ...
  def loadJobQueueRangesAndSizes(limitSites: i32): Map[SiteId, (TimeRange, i32)] = {
    _loadJobQueueStatsImpl(mustHaveRange = true, limitSites = limitSites) map {
          case (siteId, rangeAndSize) =>
      val range = rangeAndSize._1.getOrDie("TyE5KJ20FW4", s"No range for site $siteId")
      val size = rangeAndSize._2
      siteId -> (range, size)
    }
  }


  def loadJobQueueAnyRangesAndSizes(limitSites: i32): Map[SiteId, (Opt[TimeRange], i32)] = {
    _loadJobQueueStatsImpl(mustHaveRange = false, limitSites = limitSites)
  }


  private def _loadJobQueueStatsImpl(mustHaveRange: Bo, limitSites: i32)
          : Map[SiteId, (Opt[TimeRange], i32)] = {
    ...
    val join = if (mustHaveRange) "inner join" else "left outer join"
    val query = s""" -- _loadJobQueueStatsImpl
        with
          queue_lengths as (
              select
                  site_id,  count(*) as queue_len
              from  job_queue_t
              group by site_id
              limit $limitSites),
          time_ranges as (
              select
                    site_id,  time_range_to_c,  time_range_to_ofs_c
                from  job_queue_t
                where do_what_c = 1  -- for now, means indexing
                  -- There's one or none per site, see this unique constr:
                  -- jobq_u_dowhat_1_site_timerange.
                  and time_range_to_c is not null)
        select * from
            queue_lengths  $join  time_ranges  using (site_id)  """
