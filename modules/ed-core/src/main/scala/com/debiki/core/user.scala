...
sealed trait MemberInclDetails {
  ...
  /** UI features to enable or disable, or which UI variant to use. For A/B testing and
    ...
    */
  def uiVariants: Option[JsObject]

  def copyTrait(uiVariants: Option[JsObject] = null): MemberInclDetails = {
    this match {
      case g: Group =>
        g.copy(
          uiVariants = if (uiVariants ne null) uiVariants else g.uiVariants)
      case u: UserInclDetails =>
        u.copy(
          uiVariants = if (uiVariants ne null) uiVariants else u.uiVariants)
    }
  }
}
...
case class UserInclDetails(
  ...
  uiVariants: Option[JsObject] = None,
  isOwner: Boolean = false,
  isAdmin: Boolean = false,
  isModerator: Boolean = false,
  ...
  uiVariants.flatMap(anyWeirdJsObjField) foreach { problemMessage =>
    die("TyE2AKBS04", s"User with weird uiVariants JSON field: $problemMessage")
...
case class Group(
  ...
  uiVariants.flatMap(anyWeirdJsObjField) foreach { problemMessage =>
    die("TyE2AKBS05", s"Group with weird uiVariants JSON field: $problemMessage")
  }
