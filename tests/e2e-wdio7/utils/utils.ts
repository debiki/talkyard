...
/// For blog comments SSO. Encrypts one's credentials in a PASETO token,
/// by calling out to a small help tool, paseto-cmd.
///
...
export function encryptLocalPasetoV2Token(sharedSecret: St, msgObj: any): St {
  ...
  // If invoked via  s/tyd  then the current dir is the repo root,
  // but if via  s/wdio-7,  then it's  tests/e2e-wdio7/  currently.
  // So, there's a softlink from the latter to 'modules', so this
  // link works:
  const cmd = './modules/paseto-cmd/target/debug/paseto-cmd ' +
                `'${secretNoHexPrefix}' ` +
                `'${messageAsSt}'`;
  // (If there's an error, paseto-cmd might print nothing or some cryptic Rust weirdness :-/)
...
export function makeEmbeddedCommentsHtml(ps: {
      embedAsForum?: Bo,
      pageName: string, discussionId?: string,
    ...
    const forumOrComments = ps. embedAsForum ? `forum:` : `comments`;
    ...
    const talkyardPathLine = !ps.talkyardPath ? '' :
            `talkyardPath = '${ps.talkyardPath}';`;
...
talkyardAuthnToken = ${JSON.stringify(ps.authnToken)};
    ...
    const scriptUrl = `${settings.scheme}://${ps.localHostname}.localhost/-/` + (
                    ps.embedAsForum ? 'talkyard-forum.js' : 'talkyard-comments.js');

    let resultHtmlStr = `
<!DOCTYPE html>
<html>
<head>
<title>Embedded ${forumOrComments} E2E test</title>
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
</head>
<body style="background: ${ps.bgColor || 'black'}; color: ${ps.color || '#ccc'}; font-family: monospace; font-weight: bold;">
<p>Embedded ${forumOrComments} E2E test page "${ps.pageName}".<br>
...
<div class="${ps.embedAsForum ? 'talkyard-forum' : 'talkyard-comments'
                }" ${discIdAttr} ${catRefAttr} style="margin-top: 45px;"></div>
`}
${ps.appendExtraHtml || ''}
<hr>
<p>/End of page.</p>
