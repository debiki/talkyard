      ...
      preferences: {  // RENAME to prefs
        ...
        security: {
          ...
          waitForLoaded: async (): Pr<V> => {
            await this.waitForVisible('.c_SessL');
          },
  ...
  hackServer = {
    setSidPart12ViaJs: async (value: St, ps: { inCookie?: true, inField?: true }): Pr<V> => {
      await this.execute(function(value, ps) {
        if (ps.inCookie) {
          die("No TyCoSid12 — it's 123 now");
          console.debug(`Setting cookie TyCoSid12 = "${value}" ...`);
          window['getSetCookie']('TyCoSid12', value);
          console.debug(`Done.`);
        }
        if (ps.inField) {
          try {
      ...
      const anyValue = await this.#br.execute(function(ps) {
        ...
        if (ps.inField) {
          try {
            ...
            result = {
              sidFieldParts12Maybe3: window['debiki2'].getMainWin().typs.weakSessionId, // RENAME to tySid123?
            };
            console.debug(`Done.`);
          }
          catch (ex) {
    ...
    loadEmailAddressesAndLoginMethods: async (): Pr<A> => {
      const result: A = await this.executeAsync(function(done) {
        const store = window['theStore'];
        const me = store.me;
    ...
    addEmailAddresses: async (newAdr: St): Pr<A> => {
      const result: A = await this.executeAsync(function(newAdr, done) {
        const store = window['theStore'];
        const me = store.me;
    ...
    suspendUser: async (userId: PatId): Pr<A> => {
      const result: A = await this.executeAsync(function(userId, done) {
        // Calls:  /-/suspend-user
        // handled by:  UserController.suspendUser().
        window['debiki2'].Server.suspendUser(
              userId, 123, "No reason",
              function onOk(resp) {
                console.debug(`OK response: ${JSON.stringify(resp)}`);
                done({ serverSaysOk: resp });
              },
