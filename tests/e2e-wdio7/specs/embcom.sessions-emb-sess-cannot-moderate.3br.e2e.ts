...
import { logMessage, j2s } from '../utils/log-and-die';
...
let localStorageSessionItem = '';

let stolenSidPart1OnlyWasMajas = '';
let stolenSidPart1And2WasMajas = '';

let stolenSidPart1OnlyWasMons = '';
...
describe(`embcom.sessions-emb-sess-cannot-moderate.3br  TyTEECSESS0MOD`, () => {
  ...
  it(`Mallory steals the session somehow`, async () => {
    localStorageSessionItem = await mons_brA.execute(function() {
      return localStorage.getItem('talkyardSession');
    });

    stolenSidPart1And2WasMons = JSON.parse(localStorageSessionItem).weakSessionId;

    logMessage(`Mallory stole Mons' session: "${localStorageSessionItem}",
        part 1 + 2: "${stolenSidPart1And2WasMons}"`);
  });


  it(`Mallory goes to the embedding page he too (in his own browser B)`, async () => {
    await mallory_brB.go2(embeddingPageUrl);
  });

  it(`... puts the stolen session in localStorage`, async () => {
    await mallory_brB.execute(function(localStorageSessionItem) {
      localStorage.setItem('talkyardSession', localStorageSessionItem);
    }, localStorageSessionItem);
  });
  ...
  it(`Bad 5: Mallory tries to download Mons' personal data`, async () => {
    await mallory_brB.switchToEmbCommentsIframeIfNeeded();  // reidr
    resp = await mallory_brB.hackServer.loadEmailAddressesAndLoginMethods();
  });
  ...
  it(`... but in the new tab, he's not logged in — cookiest missing`, async () => {
  ...
  it(`... his name and emai (among other things) are in the JSON`, async () => {
    const source = await mons_brA.getPageSource();      // fok
  ...
  it(`Mallory tries to fetch Mon's posts`, async () => {
    await mallory_brB.go2(monsContentUrl, {
            willBeWhere: IsWhere.External });
  });
