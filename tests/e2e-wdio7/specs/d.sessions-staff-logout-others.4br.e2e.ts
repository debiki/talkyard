...
describe(`d.sessions-staff-logout-others.4br  TyTESESLGOOTR`, () => {
  ...
  it(`Mallory types the URL path to Maja's Security tab`, async () => {
  ...
  it(`... sees the Preferences tab  ttt`, async () => {
    await mons_brA.userProfilePage.waitForTabsVisible();      // fok
    assert.that(await mons_brA.userProfilePage.tabs.isPreferencesTabDisplayed());
  });
  it(`... and the Security tab  ttt`, async () => {
  ...
  it(`Maja is still logged in: She can submit a topic`, async () => {
    await maja_brB.complex.createAndSaveTopic({ title: "Maja's Topic",
            body: "Milk cream, everyone, for dinner, have should." });
  });
  ...
  it(`... she's still logged in — sees his username menu`, async () => {
  ...
  it(`... but there's no Security tab! Cannot view admin's sessions`, async () => {
  ...
  it(`... there's an error: he's not allowed to view an admin's sessions`, async () => {
    await mons_brA.serverErrorDialog.waitAndAssertTextMatches('TyEADMINSESS');
