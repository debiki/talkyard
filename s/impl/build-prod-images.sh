...
if [ -z "$skip_restart" ]; then
  sudo $test_containers kill web app search cache rdb backup
...
if [ -z "$skip_build" ]; then
  ...
  s/d build nodejs # so we'll use an up-to-date image when compiling Typescript
  s/d-gulp clean
  s/d-gulp build_release_dont_clean_before

  # Build images (except for the app server prod image). This will Dockerfile COPY
  # js and css generated by 'gulp build_...' above into 'web'.
  s/d build app # dev build, so we'll use the correct Ty image when running `test dist`.
  s/d build web
  s/d build cache
  s/d build rdb
  s/d build search
  ...
  # Then run `dist` to build the JAR file with Talkyard's compiled Scala code, which
  # we'll copy into the prod image.
  PLAY_HEAP_MEMORY_MB=8192 IS_PROD_TEST=true s/d-cli clean compile test:compile
  PLAY_HEAP_MEMORY_MB=8192 IS_PROD_TEST=true s/d-cli test dist
  s/d kill web app
  s/d down
