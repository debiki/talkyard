...
const SecurityPrefsTab = React.createFactory<any>(function(props: {
  ...
  const [sessionsOrNull, setSessions] = React.useState<Session[] | Nl>(null);
...
function SessionInfo(session: Session, endSessionFn: (ps: { session: Session }) => V, me: Me) {
  ...
  const isCurrent = me.mySidPart1ForJson !== session.part1ForJson ? '' :
          " — this session, here";
  ...
  if (deletedAt) {
    ...
    terminateBtn = me.mySidPart1ForJson === session.part1ForJson ? null :
