...
/* Parts Copyright Google Inc — most of the file.
 * Parts Copyright Kaj Magnus Lindberg — minor changes.
...
function googleCajaSanitizeHtml(htmlTextUnsafe, allowClassAndIdAttr,
    allowDataAttr,
    // Is of Javascript type St[]. And in Scala, it's Sec[St].
    relFollowTo) {
  ...
  sanitizeHtmlConfig.transformTags = {
    'a': function(tagName, attribs) {
      ...
      var relFollow = -1; // means No, add rel=nofollow
      ...
      if (relFollow === -1 && relFollowTo && window.nashorn_shallFollowUrl) {
        ...
        for (var i = 0; i < relFollowTo.length; ++i) {
          ...
          var followTo = relFollowTo[i];
          //console.debug('attribs.href = ' + attribs.href  + ', followTo = ' + followTo);
          if (window.nashorn_shallFollowUrl(attribs.href, followTo)) {
            //console.debug('relFollow —> 1');
            relFollow = 1; // yes, follow this link  (by not adding nofollow)
          }
        }
