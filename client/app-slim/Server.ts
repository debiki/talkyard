...
///
export function getCurSid12Maybe3(): St | Nl {  // [ts_authn_modl]
  ...
  if (!sid) {
    // Cannot use store.me.mySidPart1ForJson — we might not yet have loaded
    // ourselves from the server; me might be stale.
...
export function deleteTempSessId() {  // [ts_authn_modl]
  // Need not delete store.me.mySidPart1ForJson — we'll reload the page anyway. [is_logging_out]
  const mainWin = getMainWin();
  const typs: PageSession = mainWin.typs;
...
export function loadMyself(onOk: (me: Me | NU, stuffForMe?: StuffForMe) => Vo) {
  ...
  get(`/-/load-my-page-data?pageIds=${pageIds}`,
    ...
    // ! onErr(send 'failedToLogin' to parent frame)
}


export function listSessions(patId: PatId,
      onOk: (response: { sessions: Session[] }) => Vo, onError: () => Vo) {
  get(`/-/list-sessions?patId=${patId}`, onOk, onError);
}


export function terminateSessions(
        ps: { forPatId: PatId, sessionsStartedAt?: WhenMs[], all?: true },
        onOk: (response: TerminateSessionsResponse) => Vo, onError: () => Vo) {
  postJsonSuccess(`/-/terminate-sessions`, onOk, ps, onError);
}


export function listDrafts(userId: UserId,
      onOk: (response: ListSessionsResponse) => void, onError: () => void) {
