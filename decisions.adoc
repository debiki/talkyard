...
Talkyard uses Nashorn for server side rendering. So, right now, can upgrade only
to Java 11.
...
can continue working unaffected.  (The app server would send a HTTP? WebSocket? message
...
needed for Deno to render the whole thing.)
