...
class SiteDao(
  ...
  // Add  retryIfSerializationFailure  ?
...
object SiteDao {
  ...
  def withManySiteWriteLocks[R](siteIds: Set[SiteId])(block: => R): R = {
    // Lock in same order, to avoid deadlocks.
    //val idsSorted = siteIds.toSeq.sorted
    //lockManyImpl(idsSorted) {   // <—— no longer needed, instead, always calling
                  // dangerous_readWriteTransaction before, which
                  // takes a whole db write lock  (withWholeDbWriteLock { ... }).
      block
    //}
  }
