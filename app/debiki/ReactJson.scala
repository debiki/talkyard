...
object ReactJson {
  ...
  def makeSettingsVisibleClientSideJson(settings: EffectiveSettings): JsObject = {
    ...
    if (settings.singleSignOnEnabled != D.singleSignOnEnabled)
      json += "singleSignOnEnabled" -> JsBoolean(settings.singleSignOnEnabled)
    if (settings.singleSignOnSecret != D.singleSignOnSecret)
      json += "singleSignOnSecret" -> JsString(settings.singleSignOnSecret)
    if (settings.singleSignOnLoginUrl != D.singleSignOnLoginUrl)
      json += "singleSignOnLoginUrl" -> JsString(settings.singleSignOnLoginUrl)
    if (settings.singleSignOnLogoutUrl != D.singleSignOnLogoutUrl)
      json += "singleSignOnLogoutUrl" -> JsString(settings.singleSignOnLogoutUrl)
    if (settings.showExperimental != D.showExperimental)
      json += "showExperimental" -> JsBoolean(settings.showExperimental)
    if (settings.embeddedOnlyHideSite != D.embeddedOnlyHideSite)
      json += "embeddedOnlyHideSite" -> JsBoolean(settings.embeddedOnlyHideSite)
